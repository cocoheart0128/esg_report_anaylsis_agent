...
def run_etl_pipeline(start_yr="2024", end_yr="2026", isu_cd="282330"):
    ...
    # 🌟 수정됨: acpt_no1(지속가능경영) 또는 acpt_no2(지배구조) 중 하나라도 있는 행만 필터링
    valid_rows = []
    for _, row in df.iterrows():
        has_no1 = pd.notna(row.get('acpt_no1_doc_json')) and str(row.get('acpt_no1_doc_json')).strip() != ""
        has_no2 = pd.notna(row.get('acpt_no2_doc_json')) and str(row.get('acpt_no2_doc_json')).strip() != ""
        if has_no1 or has_no2:
    ...
    if all_chunks:
        ...
        # 🌟 에러 해결 핵심: 데이터 적재 직후 FTS(Full Text Search) 인덱스 생성
        try:
            print("\n=== 🔍 벡터 DB 검색 최적화(FTS 인덱스 생성) 시작 ===")
            # db = lancedb.connect("/app/data/esg_lancedb")
            # tbl = db.open_table("tb_esg_corp_gov_report")
            
            # 'text' 컬럼을 기준으로 역색인(Inverted Index) 생성
            loader.create_fts_index(column_name = "text", table_name = "tb_esg_corp_gov_report")
            print("✅ 텍스트 컬럼(text)에 FTS 인덱스 생성 완료! 이제 하이브리드 검색이 가능합니다.")
        except Exception as e:
            print(f"⚠️ FTS 인덱스 생성 중 오류 발생: {e}")
...
def check_lancedb_data():
    ...
    if "tb_esg_grade_info" in tables:
        ...
        print(f"컬럼 정보: {df_grade.columns.tolist()}")
